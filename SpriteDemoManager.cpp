...
/* load an image file directly as a new OpenGL texture */
	mSpriteTextureMap = SOIL_load_OGL_texture("Kirby.png", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID,
...
void SpriteDemoManagerC::renderSprites()
{
	...
	for(int y=0; y < mHeight; ++y)
	{
		...
		{	
			GLfloat xPositionLeft = -1750.0f;
			GLfloat xPositionRight = -1500.0f;



			GLfloat yPositionTop = 1500.0f;
			GLfloat yPositionBottom = 1750.0f;

			GLfloat xTextureCoord = getSpriteID(x, y) * 0.00001f;
			GLfloat yTextureCoord =  0.0f;


			glColor4ub(0xFF, 0xFF, 0xFF,0xFF);
			glTexCoord2f (xTextureCoord, yTextureCoord);
			glVertex3f (xPositionLeft, yPositionTop, 0.0);
			glTexCoord2f (xTextureCoord + (0.15f), yTextureCoord);
			glVertex3f (xPositionRight, yPositionTop, 0.0);
			glTexCoord2f (xTextureCoord + (0.15f), yTextureCoord + (1.0f));
			glVertex3f (xPositionRight, yPositionBottom, 0.0);
			glTexCoord2f (xTextureCoord, yTextureCoord + (1.0f));
...
void SpriteDemoManagerC::randomizeSprites()
{
	for(int y=0; y < mHeight; ++y)
	{
		for(int x=0; x < mWidth; ++x)
		{
			setID(x,y,x);
			setMood(x,y,x);
			float xPos = (float)(x-mWidth/2)*30.0f;
			float yPos = (float)(y-mHeight/2)*12.0f;
