...
Sprite::Sprite(float_t xPosition, float_t yPosition, uint16_t width, uint16_t height, char8_t* spriteName, uint16_t spriteLayer)
{

	setPosition(xPosition, yPosition);
	mWidth = width;
	mHeight = height;
	mSpriteLayer = spriteLayer;



	mSpriteTextureID = SOIL_load_OGL_texture(spriteName, SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT);
}

void Sprite::init(GL_Window* window)
{
	mRotation = 0;
	setVelocity(0, 0);
	enableSprite();
...
void Sprite::ChangeSprite(char8_t* newSpriteName)
{
	mSpriteTextureID = SOIL_load_OGL_texture(newSpriteName, SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT);
}
