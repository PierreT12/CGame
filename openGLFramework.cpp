...
void RestartGame(GL_Window* window)
{
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    *stdin = *hf_in;
	...
	char8_t				title[24];
	strncpy(title,CGame::GetInstance()->GetGameTitle(),22);
	...
	window.init.width			= Manager::mScreenWidth;						// Window Width
	window.init.height			= Manager::mScreenHeight;						// Window Height
	window.init.bitsPerPixel	= Manager::mBitsPerPixel;						// Bits Per Pixel
				...
				while (isMessagePumpActive == TRUE)						// While The Message Pump Is Active
				{
					...
					else												// If There Are No Messages
					{
						...
						else											// If Window Is Visible
						{
							...
							Manager::GetInstance()->UpdateFrame(tickCount - window.lastTickCount, &window);  // Update The Counter and pass in window so the viewport can be modded as needed
							if (Manager::GetInstance()->misGameOver == TRUE)
							{
								ReshapeGL(window.init.width, window.init.height);
							}
							window.lastTickCount = tickCount;			// Set Last Count To Current Count
							Manager::GetInstance()->DrawScene(&window);		// Draw Our Scene
